...
namespace Holding.Base.Cache
{
    ...
    public class CacheProvider : ICacheProvider
    {
        ...
        public T Get<T, TM>(string cacheKey, TimeSpan slidingExpiryWindow, Func<T> getData) where T : class
        {
            CacheKeyDependency.Add<TM>(cacheKey);
            return Get<T>(cacheKey, slidingExpiryWindow, getData);
        }
        ...
        public void InvalidateCacheItem<TM>()
        {
            if (!_config.IsCacheEnabled)
            {
                return;
            }
            if (CacheKeyDependency.CacheKeies.ContainsKey(typeof (TM)))
            {
                CacheKeyDependency.CacheKeies[typeof(TM)].ToList().ForEach(_cache.InvalidateCacheItem);
            }
        }
